...
public abstract class AbsractLocalContext<T extends AbstractInstanceType, P extends AbstractTermPrice<T>, C extends AbstractAwsEc2Price, Q extends AbstractQuoteVm<P>>
	 ...
	 */
	@Getter
	private Map<String, C> partialCost = new HashMap<>();

	/**
	 * The previous installed local prices. Key is the code.
	 */
	@Getter
	private Map<String, P> locals;
	...
	@Getter
	private BaseProvInstanceTypeRepository<T> tRepository;

	@Getter
	private BaseProvTermPriceRepository<T, P> pRepository;

	@Getter
	private BaseProvQuoteRepository<Q> qRepository;
	 ...
	 */
	protected AbsractLocalContext(final UpdateContext parent, final BaseProvInstanceTypeRepository<T> tRepository,
			final BaseProvTermPriceRepository<T, P> pRepository, final BaseProvQuoteRepository<Q> qRepository,
			final ProvLocation region, final Map<String, T> previousTypes, final String term1, final String term2) {
		super(parent);
