...
function installMaven {
  ...
  if [ ! -d "apache-maven-3.6.3" ]; then
    echo "Download Maven 3.6.3"
    curl -sSL http://apache.mirrors.ovh.net/ftp.apache.org/dist/maven/maven-3/3.6.3/binaries/apache-maven-3.6.3-bin.tar.gz | tar zx -C ~/maven
  fi
  popd > /dev/null
...
BUILD)
  ...
  if [ "$TRAVIS_BRANCH" == "master" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
    ...
    mvn clean package jacoco:report sonar:sonar \
          $MAVEN_ARGS \
          -Pjacoco -Djacoco.includes=org.ligoj.app.plugin.prov.aws.* \
